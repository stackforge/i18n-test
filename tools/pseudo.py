...
def translate(segment,translation):
    prefix = u""
    ...
    return u"{0}[~{1}~{2}{3}]".format(prefix, segment, translation.decode('utf-8'), extra_chars)
...
def main():
    ...
    parser.add_argument('--string', default='您好яшçあ', help='pseudo string')
    ...
    for msg in pot_cat:
        if msg.pluralizable:
            msg.string = [translate(u"{}:{}".format(i, msg.id[0]), args.string)
                          for i in range(num_plurals)]
        else:
            msg.string = translate(msg.id, args.string)
        new_cat[msg.id] = msg
